...
{
    public partial class FormGruppa : Form
    {
        ...
        private void FormGruppa_Load(object sender, EventArgs e)
        {
            ...
            adapter = new SqlDataAdapter("SELECT * FROM Gruppa", sqlConnection);
            table = new DataTable();
            adapter.Fill(table);
            ...
            dataGridView1.Columns[1].HeaderCell.Value = "Группа";
        }

        private void buttonINSStudent_Click(object sender, EventArgs e)
        {
            this.Hide();
            FormINSGruppa f1 = new FormINSGruppa();
            f1.ShowDialog();
            this.Close();
        }
        ...
        {
            MessageBox.Show("Вы точно хотите удалить данные группы?", "Предупреждение", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);

            if (!string.IsNullOrEmpty(textBoxIDDELStudent.Text) && !string.IsNullOrWhiteSpace(textBoxIDDELStudent.Text))
            {
                SqlCommand command = new SqlCommand("DELETE FROM [Gruppa] WHERE [ID_grup]=@ID_grup", sqlConnection);

                command.Parameters.AddWithValue("ID_grup", textBoxIDDELStudent.Text);
        ...
        private void buttonUPStudent_Click(object sender, EventArgs e)
        {
            ...
            {
                SqlCommand command = new SqlCommand("UPDATE [Gruppa] SET [Name]=@Name WHERE [ID_grup]=@ID_grup", sqlConnection);
        ...
        {
            if ((e.KeyChar >= 'А' && e.KeyChar <= 'Я') || (e.KeyChar >= 'а' && e.KeyChar <= 'я') || e.KeyChar == '-' || (e.KeyChar >= '0' && e.KeyChar <= '9') || e.KeyChar == (char)Keys.Back)
            {
