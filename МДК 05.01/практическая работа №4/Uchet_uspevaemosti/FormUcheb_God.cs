...
{
    public partial class FormUcheb_God : Form
    {
        private SqlConnection sqlConnection = null;
        public FormUcheb_God()
        {
            InitializeComponent();
        }

        private void FormUcheb_God_Load(object sender, EventArgs e)
        {
            ...
            adapter = new SqlDataAdapter("SELECT * FROM Ucheb_god", sqlConnection);
            table = new DataTable();
            adapter.Fill(table);
            ...
            dataGridView1.Columns[1].HeaderCell.Value = "Учебный год";
        }

        private void buttonINSStudent_Click(object sender, EventArgs e)
        {
            this.Hide();
            FormINSUcheb_God f1 = new FormINSUcheb_God();
            f1.ShowDialog();
            this.Close();
        }
        ...
        {
            MessageBox.Show("Вы точно хотите удалить данные учебного года?", "Предупреждение", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);

            if (!string.IsNullOrEmpty(textBoxIDDELStudent.Text) && !string.IsNullOrWhiteSpace(textBoxIDDELStudent.Text))
            {
                SqlCommand command = new SqlCommand("DELETE FROM [Ucheb_god] WHERE [Ucheb_god_ID]=@Ucheb_god_ID", sqlConnection);

                command.Parameters.AddWithValue("Ucheb_god_ID", textBoxIDDELStudent.Text);
        ...
        private void buttonUPStudent_Click(object sender, EventArgs e)
        {
            ...
            {
                SqlCommand command = new SqlCommand("UPDATE [Ucheb_god] SET [Name]=@Name WHERE [Ucheb_god_ID]=@Ucheb_god_ID", sqlConnection);
